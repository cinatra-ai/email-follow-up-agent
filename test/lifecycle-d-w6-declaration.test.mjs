...
test("6a — the typed produces entry on the email body kind is kept", () => {
  assert.deepEqual(cinatra.produces, [{ extension: EMAIL, objectTypeId: BODY }]);
});

test("6a — the produced extension is a required artifact dependency", () => {
  const edge = artifactDependencies.find((d) => d.packageName === EMAIL);
  assert.ok(edge, "no artifact dependency on " + EMAIL);
  assert.equal(edge.requirement, "required");
});

test("6a — a terminal binding resolves the declaration", () => {
  const bound = bindings.filter((b) => b.binding.extension === EMAIL);
  assert.equal(bound.length, 1);
  assert.equal(bound[0].output, "followupDigest");
});

test("6d — the campaign is a visible pick, since no flow supplies it", () => {
  const meta = startMeta().metadata.cinatra;
